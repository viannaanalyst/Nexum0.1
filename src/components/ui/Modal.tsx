...
export const Modal: React.FC<ModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div 
        ref={modalRef}
        className="relative z-10 w-full max-w-md bg-[#0a0a1a]/80 backdrop-blur-xl border border-white/10 rounded-[22px] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-center justify-between p-6 pb-2 relative z-20">
          <div className="flex items-center gap-4">
            ...
            <h3 id="modal-title" className="text-lg font-medium text-white/90">
              {title}
            </h3>
          </div>
          <button 
        ...
        <div className="p-6 pt-2 relative z-20">
          <p className="text-gray-400 text-sm leading-relaxed font-light mb-5 ml-1">
            {message}
          </p>
          ...
          {type === 'prompt' && (
            <input
              ...
              className="w-full bg-white/[0.03] hover:bg-white/[0.06] border border-white/[0.08] rounded-xl px-4 py-3 text-white/90 placeholder-gray-600 focus:bg-white/[0.08] focus:border-primary/30 focus:ring-0 outline-none transition-all duration-300 text-sm font-light shadow-inner"
        ...
        <div className="flex justify-end gap-3 p-5 bg-[#0a0a1a]/50 border-t border-white/5 relative z-20">
          ...
          <button
            ...
            className={`px-6 py-2.5 text-sm font-medium text-white rounded-xl shadow-lg transition-all transform hover:-translate-y-0.5 active:translate-y-0 ${
                variant === 'danger' ? 'bg-red-500/80 hover:bg-red-500 shadow-red-500/20' :
